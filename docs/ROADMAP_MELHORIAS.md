...
| 27.3 Postura mesa (Flip meio aberto) | ✔ feito | Botão PTT na metade de baixo e status na de cima, via `FoldingFeature` do `androidx.window` com `expect/actual`, sem efeito no iOS | Sem `expect/actual`: o design system tem um `LocalTabletopFold` (a altura da dobra, nulo por padrão) e só o `MainActivity` o preenche, a partir do `WindowInfoTracker`. Postura mesa é dobra meio aberta atravessando a tela (`tabletopFold`, com `TabletopFoldTest` em Robolectric); segurar como livro, com a dobra de cima a baixo, não conta. Nessa postura o PTT põe barra do topo, status e participantes acima da dobra, e botão, dica e "Sair do canal" abaixo. A barra do topo do canal foi para `ChannelTopBar.kt` para caber nos limites do detekt. Dependência nova: `androidx.window` 1.5.1 (e `window-testing` nos testes). Não verificado em aparelho: conferir no preview `PttScreenPreviewTabletop` e num Flip | M |
