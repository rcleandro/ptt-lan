...
**Roteiro manual da 24.4** (três aparelhos na mesma Wi-Fi; a 24.5 precisa estar ligada para o app hospedar, ou
use um build de teste): (1) o celular A hospeda com PIN; (2) B e C acham "PTT-LAN-<nome>" na lista e entram com o
...
`WifiLock` de baixa latência enquanto hospeda). O `PttForegroundService` atual só sobe com um canal ativo, então
a 24.5 precisa mantê-lo enquanto houver servidor hospedado, mesmo fora de canal.
